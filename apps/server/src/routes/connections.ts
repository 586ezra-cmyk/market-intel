...
// Version: v4 — yloc sticky labels + noise reduction
const PINE_SCRIPT = `// ============================================================
...
// Labels for climax
if sellingClimax and isKZ
    label.new(bar_index, na, "💥 SC",
              yloc=yloc.belowbar,
...
if buyingClimax and isKZ
    label.new(bar_index, na, "💥 BC",
              yloc=yloc.abovebar,
...
if showStruct
    if bullBOS
        label.new(bar_index, na, "BOS ▲",   yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.green,  20), textcolor=color.white, size=size.tiny)
    if bullCHoCH
        label.new(bar_index, na, "CHoCH ▲", yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.lime,   10), textcolor=color.black, size=size.tiny)
    if bearBOS
        label.new(bar_index, na, "BOS ▼",   yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.red,    20), textcolor=color.white, size=size.tiny)
    if bearCHoCH
        label.new(bar_index, na, "CHoCH ▼", yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.orange, 10), textcolor=color.black, size=size.tiny)
...
if bullIFVG
    label.new(bar_index, na, "iFVG ▲", yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.teal, 30), textcolor=color.white, size=size.tiny)
if bearIFVG
    label.new(bar_index, na, "iFVG ▼", yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.teal, 30), textcolor=color.white, size=size.tiny)
...
if showLiq
    ...
    if sweepH
        label.new(bar_index, na, "⚡ Sweep H", yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.red,   30), textcolor=color.white, size=size.tiny)
    if sweepL
        label.new(bar_index, na, "⚡ Sweep L", yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.green, 30), textcolor=color.white, size=size.tiny)
...
bearSMT  = showSMT and isKZ and not na(sPH1) and not na(pPH1) and sPH1 > pPH1 and (sPH1 - pPH1) > atr * 0.3 and not na(sPH2) and sPH2 <= pPH2
bullSMT  = showSMT and isKZ and not na(sPL1) and not na(pPL1) and sPL1 < pPL1 and (pPL1 - sPL1) > atr * 0.3 and not na(sPL2) and sPL2 >= pPL2
hasSMT   = bearSMT or bullSMT
smtDir   = bearSMT ? "bearish" : "bullish"

if bearSMT and dispMidTF
    label.new(bar_index, na, "⚡ SMT ▼", yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.fuchsia, 20), textcolor=color.white, size=size.small)
if bullSMT and dispMidTF
    label.new(bar_index, na, "⚡ SMT ▲", yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.fuchsia, 20), textcolor=color.white, size=size.small)

// ─── iSMT (2-candle) ─────────────────────────────────────────────────────────
validISMTtf = timeframe.in_seconds() <= 3600
bearISMT = showISMT and validISMTtf and isKZ and high  > high[1] and close  < high[1] and not (b_h > b_h[1] and b_c > b_h[1])
bullISMT = showISMT and validISMTtf and isKZ and low   < low[1]  and close  > low[1]  and not (b_l < b_l[1] and b_c < b_l[1])
hasISMT  = bearISMT or bullISMT
ismtDir  = bearISMT ? "bearish" : "bullish"

if bearISMT
    label.new(bar_index, na, "⚡ iSMT ▼", yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.fuchsia, 10), textcolor=color.white, size=size.small)
if bullISMT
    label.new(bar_index, na, "⚡ iSMT ▲", yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.fuchsia, 10), textcolor=color.white, size=size.small)
...
if spring and dispMidTF
    label.new(bar_index, na, "🌱 Spring",    yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.green, 20), textcolor=color.white)
if upthrust and dispMidTF
    label.new(bar_index, na, "⬆ Upthrust",   yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.red,   20), textcolor=color.white)
if wyckoffSC and dispMidTF
    label.new(bar_index, na, "📉 Wyckoff SC", yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.red,   10), textcolor=color.white, size=size.normal)
if wyckoffBC and dispMidTF
    label.new(bar_index, na, "📈 Wyckoff BC", yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.green, 10), textcolor=color.white, size=size.normal)
...
if (judasBull or nyJBull) and dispIntraday
    label.new(bar_index, na, "🎭 Judas ▲", yloc=yloc.belowbar, style=label.style_label_up,   color=color.new(color.teal, 20), textcolor=color.white, size=size.small)
if (judasBear or nyJBear) and dispIntraday
    label.new(bar_index, na, "🎭 Judas ▼", yloc=yloc.abovebar, style=label.style_label_down, color=color.new(color.teal, 20), textcolor=color.white, size=size.small)
