...
rangePos   = not na(rangeH) and not na(rangeL) and (rangeH - rangeL) > 0 ?
             math.round((close - rangeL) / (rangeH - rangeL) * 100) : 50
...
alertcondition(hasIFVG, "iFVG (Inverse FVG Retest)",
    '{"secret":"' + secret + '","event":"ifvg","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period + '","time":' + str.tostring(math.round(time)) +
    ',"direction":"' + ifvgDir + '","price":' + str.tostring(close) + '}')
...
alertcondition(spring or wyckoffSC, "Wyckoff Accumulation (Spring/SC)",
    '{"secret":"' + secret + '","event":"wyckoff","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period + '","time":' + str.tostring(math.round(time)) +
    ',"phase":"accumulation","subtype":"' + (wyckoffSC ? "selling_climax" : "spring") +
    '","volRank":' + str.tostring(math.round(volRank)) + ',"confidence":0.80}')

alertcondition(upthrust or wyckoffBC, "Wyckoff Distribution (Upthrust/BC)",
    '{"secret":"' + secret + '","event":"wyckoff","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period + '","time":' + str.tostring(math.round(time)) +
    ',"phase":"distribution","subtype":"' + (wyckoffBC ? "buying_climax" : "upthrust") +
    '","volRank":' + str.tostring(math.round(volRank)) + ',"confidence":0.80}')

alertcondition(hasOB, "Order Block",
    '{"secret":"' + secret + '","event":"order_block","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period + '","time":' + str.tostring(math.round(time)) +
    ',"direction":"' + obDir + '","price":' + str.tostring(close) + '}')

alertcondition(aboveBBupper or belowBBlower, "Bollinger Band Break",
    '{"secret":"' + secret + '","event":"bb_break","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period + '","time":' + str.tostring(math.round(time)) +
    ',"direction":"' + (aboveBBupper ? "bullish" : "bearish") +
    '","bbUpper":' + str.tostring(math.round(bbUpper, 2)) +
    ',"bbLower":' + str.tostring(math.round(bbLower, 2)) +
    ',"price":' + str.tostring(close) + '}')

// ── Volume Climax Alert ──────────────────────────────────────────────────────
alertcondition(sellingClimax or buyingClimax, "Volume Climax (SC/BC)",
    '{"secret":"' + secret + '","event":"volume_climax","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period + '","time":' + str.tostring(math.round(time)) +
    ',"type":"' + (sellingClimax ? "selling_climax" : "buying_climax") +
    '","direction":"' + (sellingClimax ? "bearish" : "bullish") +
    '","volRank":' + str.tostring(math.round(volRank)) +
    ',"price":' + str.tostring(close) +
    ',"inKillZone":' + str.tostring(isKZ) +
    ',"dealingZone":"' + dealingZone + '"}')

alertcondition(noSupply or noDemand, "Volume Low (No Supply / No Demand)",
    '{"secret":"' + secret + '","event":"volume_low","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period + '","time":' + str.tostring(math.round(time)) +
    ',"type":"' + (noSupply ? "no_supply" : "no_demand") +
    '","direction":"' + (noSupply ? "bearish" : "bullish") +
    '","volRank":' + str.tostring(math.round(volRank)) +
    ',"price":' + str.tostring(close) + '}')

// ★ MASTER confluence alert — full payload with ALL elements
alertcondition(confluenceOK, "★ CONFLUENCE ALERT ★",
    '{"secret":"' + secret +
    '","event":"confluence","symbol":"' + syminfo.ticker +
    '","timeframe":"' + timeframe.period +
    '","time":' + str.tostring(math.round(time)) +
    ',"direction":"' + confluenceDir +
    '","currentPrice":' + str.tostring(close) +
    ',"factorCount":' + str.tostring(factorCount) +
    ',"inKillZone":' + str.tostring(isKZ) +
    ',"killZone":"' + (isLondon ? "london" : isNY ? "ny" : "none") +
    '","dealingZone":"' + dealingZone +
    '","rangePosition":' + str.tostring(rangePos) +
    ',"hasBOSorCHoCH":' + str.tostring(hasBOS) +
    ',"hasLiquiditySweep":' + str.tostring(hasLiqSweep) +
    ',"hasFVG":' + str.tostring(hasFVG) +
    ',"hasIFVG":' + str.tostring(hasIFVG) +
    ',"hasSMT":' + str.tostring(hasSMT) +
    ',"hasISMT":' + str.tostring(hasISMT) +
    ',"hasWyckoff":' + str.tostring(hasWyckoff) +
    ',"wyckoffPhase":"' + wyckoffPhase +
    '","hasOB":' + str.tostring(hasOB) +
    ',"hasJudas":' + str.tostring(hasJudas) +
    ',"aboveBBupper":' + str.tostring(aboveBBupper) +
    ',"belowBBlower":' + str.tostring(belowBBlower) +
    ',"bbSqueeze":' + str.tostring(bbSqueeze) +
    ',"volRank":' + str.tostring(math.round(volRank)) +
    ',"hasVolClimax":' + str.tostring(hasVolClimax) +
    ',"volType":"' + (sellingClimax ? "SC" : buyingClimax ? "BC" : noSupply ? "NS" : noDemand ? "ND" : "normal") + '"}')
`
