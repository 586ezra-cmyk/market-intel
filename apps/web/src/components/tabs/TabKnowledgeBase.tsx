...
שימוש: לא לכנס בפתיחת סשן. לחכות לJudas Swing ואז לכנס בכיוון ההפוך.`,
  ...
  {
    id: 'wing-break', titleHe: 'שבירת כנף', titleEn: 'Wing Break',
    abbr: undefined,
    category: 'אסטרטגיות',
    summary: 'אסטרטגיית שורט בתוך Wyckoff Phase D — שבירת טרנד עולה מזויף וריטסט לפני ירידה.',
    explanation: `שבירת כנף היא אסטרטגיה שפועלת אך ורק בתוך Wyckoff Distribution Phase D.

הרצף המלא:
1. WING — טרנד נזילות עולה ב-Phase D (נראה כמו כוח, אבל זו מלכודת).
   מחיר יוצר Higher Lows בזמן שה-"כסף הגדול" מוכר בשקט.
2. LPSY — Last Point of Supply, שיא אחרון עם כוח מתדלדל.
3. שבירת הכנף — מחיר שובר את הטרנד העולה כלפי מטה.
4. ריטסט — מחיר חוזר לטרנד השבור מלמטה ("אל חזור" — לא סוגר מעלה).
5. BOS ללמטה — כניסת שורט ✅

ניהול עסקה:
SL: מעל כל מבנה ה-BOS / מעל ה-CREEK
TP1: ICE / תמיכה קרובה
TP2: שפל ה-Distribution

ההבדל בין שבירת כנף לטיסה הפוכה:
• שבירת כנף: כניסה בתוך התהליך (בריטסט על שבירת הטרנד)
• טיסה הפוכה: כניסה אחרי פריצה מה-ICE וריטסט עליו`,
    assets: 'כל הנכסים',
    timeframes: 'כל טווחי הזמן',
    requiredConfirmations: 'Wyckoff Phase D + LPSY + שבירת טרנד + ריטסט + BOS',
    commonMistakes: 'להיכנס ללא Phase D מאושר. לא לחכות לריטסט. SL קצר מדי.',
    relatedTopics: ['wyckoff', 'bos', 'choch', 'liquidity-sweep'],
  },
  {
    id: 'w-pattern', titleHe: 'תבנית W', titleEn: 'W Pattern',
    abbr: 'W',
    category: 'תבניות',
    summary: 'שתי תחתיות ברמה דומה עם קפיצה ביניהן — אות היפוך בוליש עם אישור FVG/iSMT.',
    explanation: `תבנית W היא תבנית היפוך בוליש המורכבת מ-2 שפלים ברמות דומות:

הרצף:
1. מחיר יורד לשפל ראשון (תחתית שמאל של W)
2. קפיצה חזרה — יוצרת את "האמצע" של ה-W
3. מחיר יורד שוב לשפל שני באותה רמה (תחתית ימין של W)
4. מחיר עולה ושובר את האמצע = כניסת לונג ✅

תנאי כניסה (חייב אחד מהם):
• FVG בוליש באזור השפל השני
• iSMT בוליש ליד השפל השני

ניהול:
SL: מתחת לשפל הנמוך מבין השניים
TP1: FVG קרוב מעל
TP2: Equal Highs / נזילות חיצונית

הכלל: לא נכנסים ב-W ללא אישור FVG או iSMT — אחרת זה שבירת תמיכה רגילה!`,
    assets: 'כל הנכסים',
    timeframes: 'כל טווחי הזמן',
    requiredConfirmations: 'FVG בוליש או iSMT בוליש ליד השפל השני',
    commonMistakes: 'להיכנס ב-W ללא אישור. לא לוודא שהשפלים ברמה דומה (סטייה של עד 0.2%).',
    relatedTopics: ['fvg', 'ismt', 'double-bottom', 'choch'],
  },
  {
    id: 'm-pattern', titleHe: 'תבנית M', titleEn: 'M Pattern',
    abbr: 'M',
    category: 'תבניות',
    summary: 'שתי פסגות ברמה דומה עם ירידה ביניהן — אות היפוך בארישׁ עם אישור FVG/iSMT.',
    explanation: `תבנית M היא תבנית היפוך בארישׁ — הפוך מ-W:

הרצף:
1. מחיר עולה לשיא ראשון (פסגה שמאל של M)
2. ירידה חזרה — יוצרת את "האמצע" של ה-M
3. מחיר עולה שוב לשיא שני באותה רמה (פסגה ימין של M)
4. מחיר יורד ושובר את האמצע = כניסת שורט ✅

תנאי כניסה (חייב אחד מהם):
• FVG בארישׁ באזור השיא השני
• iSMT בארישׁ ליד השיא השני (נכס A עולה, נכס B לא מאשר)

ניהול:
SL: מעל השיא הגבוה מבין השניים
TP1: FVG קרוב מתחת
TP2: Equal Lows / נזילות חיצונית

הכלל: לא נכנסים ב-M ללא אישור FVG או iSMT!`,
    assets: 'כל הנכסים',
    timeframes: 'כל טווחי הזמן',
    requiredConfirmations: 'FVG בארישׁ או iSMT בארישׁ ליד השיא השני',
    commonMistakes: 'להיכנס ב-M ללא אישור. לבלבל M עם Wyckoff Distribution.',
    relatedTopics: ['fvg', 'ismt', 'double-top', 'choch'],
  },
  {
    id: 'double-top', titleHe: 'דאבל טופ', titleEn: 'Double Top',
    abbr: undefined,
    category: 'תבניות',
    summary: 'שני שיאים כמעט זהים — אזור התנגדות חזק שמחיר לא הצליח לשבור.',
    explanation: `דאבל טופ הוא אות בארישׁ שמצביע על אזור התנגדות חזק:

מה זה:
מחיר עולה לשיא, יורד, חוזר לאותו שיא (תוך 0.1%) ואינו מצליח לפרוץ מעלה.

איך זה שונה מ-M Pattern:
• דאבל טופ = 2 שיאים זהים = אזור התנגדות / נזילות
• תבנית M = אישור מלא עם FVG/iSMT = אות כניסה

דאבל טופ כאישור:
משמש כ-אישור נוסף לאיתותים אחרים:
• שורט ICT + דאבל טופ = אישור נוסף +0.3 בדירוג
• לא בהכרח כניסה עצמאית ללא אישורים נוספים

ניהול אם כניסה:
SL: מעל שני השיאים
TP1: שפל שבין שני השיאים ("הצוואר")
TP2: פרוייקציה של גובה התבנית כלפי מטה`,
    assets: 'כל הנכסים',
    timeframes: 'כל טווחי הזמן',
    requiredConfirmations: 'CHoCH + FVG מתחת לצוואר, או iSMT',
    commonMistakes: 'להיכנס שורט רק מדאבל טופ ללא אישורים נוספים.',
    relatedTopics: ['m-pattern', 'liquidity-sweep', 'choch', 'fvg'],
  },
  {
    id: 'double-bottom', titleHe: 'דאבל בוטום', titleEn: 'Double Bottom',
    abbr: undefined,
    category: 'תבניות',
    summary: 'שני שפלים כמעט זהים — אזור תמיכה חזק שמחיר לא הצליח לשבור.',
    explanation: `דאבל בוטום הוא אות בוליש שמצביע על אזור תמיכה חזק:

מה זה:
מחיר יורד לשפל, עולה, חוזר לאותו שפל (תוך 0.1%) ואינו מצליח לפרוץ מטה.

איך זה שונה מ-W Pattern:
• דאבל בוטום = 2 שפלים זהים = אזור תמיכה / נזילות
• תבנית W = אישור מלא עם FVG/iSMT = אות כניסה

דאבל בוטום כאישור:
• משמש כ-אישור נוסף לאיתותים בוליש
• לונג ICT + דאבל בוטום = אישור נוסף +0.3 בדירוג
• Wyckoff Spring + דאבל בוטום = אות חזק מאוד

ניהול אם כניסה:
SL: מתחת לשני השפלים
TP1: שיא שבין שני השפלים ("הצוואר")
TP2: פרוייקציה של גובה התבנית כלפי מעלה`,
    assets: 'כל הנכסים',
    timeframes: 'כל טווחי הזמן',
    requiredConfirmations: 'CHoCH + FVG מעל הצוואר, או iSMT, או Wyckoff Spring',
    commonMistakes: 'להיכנס לונג רק מדאבל בוטום ללא אישורים נוספים.',
...
const CATEGORIES = ['הכל', 'מבנה שוק', 'נזילות', 'ICT', 'Wyckoff', 'SMT/iSMT', 'תבניות', 'אסטרטגיות', 'מוסדיים']
